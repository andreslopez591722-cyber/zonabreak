...
const { initDb, getKv, setKv } = require('./db');
...
// ── Menú público (solo lectura, para index.html sin login) ─
app.get('/api/public/menu', (_req, res) => {
  const recetas = getKv(db, 'zb_recetas', {});
  res.json({ recetas });
});

// ── Bootstrap (POS autenticado) ───────────────────────────
app.get('/api/bootstrap', authMiddleware, (_req, res) => {
  const ventasRows = db
    .prepare(
      `SELECT payload FROM sales ORDER BY created_at DESC LIMIT 5000`
    )
    .all()
...
app.put('/api/sales/sync', authMiddleware, (req, res) => {
  /** Reemplaza todas las ventas desde el cliente (migración / repair) */
...
app.get('/api/reports/delivery', authMiddleware, (req, res) => {
  ...
  const all = db
    .prepare(
      `SELECT payload FROM sales ORDER BY created_at DESC`
    )
    .all()
    .map((r) => JSON.parse(r.payload));
...
app.listen(PORT, () => {
  console.log(`Zona Break API + estáticos en http://localhost:${PORT}`);
